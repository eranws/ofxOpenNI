#include "JonFingerTracker.h"


JonFingerTracker::JonFingerTracker( void )
{
	g_hasFinger = false;
}

void JonFingerTracker::detectFinger( cv::Mat& handFrame, const cv::Rect& handRect )
{
	...
	if( !contours.empty() && !hierarchy.empty() )
	{
		...
		if (fingerPoint.size())
		{
			...
			if (r.x > 0 && r.y > 0 && r.x + r.width < handLabel.cols && r.y + r.height < handLabel.rows)
			{
				...
				std::vector<cv::Moments> _mu(fingContours.size() );
				std::vector<cv::Point2f> _mc( fingContours.size() );
